...
- [x] (2026-09-11 11:00Z) Owner's second round on production. Provider rename rewrites `provider:model` assignments in the same save (the server refused the settings for naming a provider that no longer existed); "the public service" reads "its own endpoint"; the operator's tool policy is the full catalog (`AgentToolsSettings.Catalog` from `agent.FullCatalog()`) by family with allowed / ask first / off per tool and per family; the activity list is a `DataTable`; `SetAgentMainConversation` (API, client, `teanode agent conversation main`) swaps a named conversation in as main or starts a fresh one, the picker pins and marks the main conversation; Starred and Priority carry unread counts (`MailboxView.starredUnread/priorityUnread` via `CountItems`) in the rail and the tab title; roles and groups rows take a press anywhere; `mail_read` has `headers` (every header line plus `MailFacts`), and the facts name a Reply-To or Return-Path elsewhere for the sorter too; the OpenAI client retries with `reasoning_effort: none` when a model refuses function tools while reasoning, remembering the model.
...
- "Did you memorize it?" (2026-09-11): the first time the owner asked on production the conduct had no Remembering section and the agent said it could not; the section shipped that evening. Tested on the dev server afterwards, the agent kept "my accountant is Dana Li" through the memory tool — addressed to `triage` alone. The conversation loads memories with `applies_to @> ["ask"]`, so that memory would never have been shown to the agent that was told it, and the next "do you remember?" would have got "nothing" again. `memoryAudiences` now always includes the conversation and drops names that are not audiences; the tool says the conversation always reads a memory and asks only which runs should too.

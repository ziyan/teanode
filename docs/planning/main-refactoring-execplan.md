...
- An in-flight graph embedding could restore a stale vector after a page or
  ...
  request instead failed the whole batch on its foreign key. Regressions
  perform each mutation inside the local provider request before returning
  vectors; the original write path fails all three cases.

- Embedding adapters mixed a startup registry with live model names and widths.
  After a pending model change, the old provider request could be stored under
  the replacement model name. Mailbox backfill also omitted the width suffix
  when checking existing vectors, requeueing completed dimensioned embeddings.
  A real-database regression reproduces the mislabeling before the correction.
...
- The HTTP query wrapper must return resolver errors from the transaction callback.
  ...
  retained only for the GraphQL response. Returning the error rolls back that field
  while allowing its sibling to execute.
...
- Decision: apply periodic graph embeddings as a guarded batch using the input
  ...
  Rationale: model calls remain outside SQL; rechecking without a lock would
  leave a second edit race. Facts depend on their page name as well as their
  own words, so both versions participate. Changed or deleted inputs are
  skipped while valid results commit; changed rows remain eligible for retry.
  Date/Author: 2026-09-21, implementation review.

- Decision: resolve embedding clients, provider model names, stored identities
  and requested widths together from the running registry.
  Rationale: model settings already require a restart; mixing pending settings
  with a startup client creates incompatible vectors under the same identity.
  Mailbox backfill must use that same dimensioned identity.
  Date/Author: 2026-09-21, implementation review.
...
Recall owns its authorization/read phases and calls its embedding provider
without an open request transaction, then revalidates account state, permission
and active agent before returning results. RecallForQuestion no longer accepts
a caller transaction. The query-scope consequence and compatibility limits are
recorded in the refactoring decision, including independent field failures and
materialized nested output objects.
...
evidence, replacement, folding and identity files. A typed rememberMaterial
carries retrieved index lines, page blocks and unlearned statements into the
prompt builder. Parsing still treats malformed model output as an empty answer.
Page creation/contact binding remains an earlier transaction; the application
helper uses the caller's transaction for facts, links, replacements and the
conversation marker. Embedding calls remain outside those transactions.
...
root authorization, known hashes, file paging, tree selection, extraction,
repository profiles, cursor interpretation, history allocation and journals.
This makes subsequent policy changes reviewable without combining them with
moves. Manifest and record readers keep their existing boundaries.
...
mail-send even after its event is deleted. Request receipts contain no message
or event content. A save replay returns current event content or null if deleted;
GetCalendarRequest provides explicit completion and an isMissing flag.
...
Rationale: separately committing participation before a legacy mail send leaves
an unanswered organizer after failure; independently sending first risks the
opposite inconsistency. Reuse event/index persistence instead of copying it into
the invitation adapter. Date: 2026-09-21.
...
ownership, row locking and writes move together; agent grant changes acquire the
same row lock. Preserve optional-field behavior and existing GraphQL names.
Rationale: a rename must not revoke a grant or commit outside its caller's
...
Revision note: calendar metadata now follows an authorized command boundary.
The pre-fix regression reproduced clearing AgentGranted during a rename and
retaining that rename after caller rollback. Update now locks the latest owned
calendar, validates timezone/week-start values, changes presentation fields and
returns the event count in the caller transaction. The agent grant adapter uses
the same row lock. Empty name and week start still retain old values, while the
other empty presentation fields still clear them.

Regressions cover the resolver rollback, concurrent grant preservation, audit
actor retention, denied principals and other owners, invalid timezone/week-start
values and optional-field behavior. Event saves and invitation dispatch remain
separate work: their durable event write and later external send require a
recoverable command boundary, not just this metadata savepoint.


Validation update: all 527 calendar, calendar-command, scheduling, DAV, API and
agent regressions pass under the race detector with the disposable database.
Focused metadata tests, repository lint including gogolint and both binary
builds pass. The preceding full-suite evidence remains 1,911 tests; this increment
...
Revision note: AnswerMailInvitation now joins the caller transaction through the
calendar SaveEvent command. It changes participation against the locked event
and accepts the REPLY message before the command can commit. A failed reply
acceptance, missing mail-send permission or caller rollback restores the original
calendar data and leaves no accepted reply or dispatch wakeup. Success retains
the existing mailbox-read/calendar-use/mail-send checks and records the reply in
the speaking address's owned mailbox. Invitations and replies share one sender
resolution helper, and the now-unused API occurrence-index helper was removed.

The regression fixture records a received invitation and injects acceptance
failure after writing mail, parent rollback and denied send permission. It checks
calendar bytes, mail count and commit callbacks together, and verifies that a
successful response shows the new participation. These are atomicity guarantees,
not replay identity: repeating an RSVP after a lost success response can still
send another reply. Explicit request identity remains part of the open recovery
work, along with new-event creation and the protocol/deployment gates.


Validation update: all 539 calendar, calendar-command, scheduling, DAV, API and
agent tests pass under the race detector. The RSVP fixture passes again after
removing the unused index helper; final lint including gogolint and both binary
...
Calendar recovery now reaches SaveCalendarEvent through an optional requestId,
with an account-scoped GetCalendarRequest lookup. Identical retries skip event
preparation and notification acceptance, changed arguments conflict, and later
edits remain intact. The Go client preserves the old query for callers without a
request ID and offers identified saves and lookup. CLI add/edit print a retained
ID before sending, accept --request-id for explicit retries, and report uncertain
errors with both lookup and retry instructions. The calendar request command
needs neither the original fields nor a surviving calendar/event.

API regressions cover duplicate invitation avoidance, preserving later edits,
deleted result lookup, revoked send permission, personal events without send
permission, failed acceptance rollback and account isolation. CLI tests inject a
lost response, check there was no automatic mutation retry, and verify an explicit
retry sends the same identity and fields. Lookup of a deleted result is a single
query. Dashboard and agent callers, RSVP and deletion identities, complete command
extraction and final deployment/Chrome verification remain required.


Validation update: the complete vector-enabled race suite passes with 1,941 tests
and one expected Chrome-endpoint skip. The affected subsystem run passes 463
...
Scanner decomposition preserves all 55 original function bodies, verified by
comparing parsed Go declarations before and after the move. Existing tests cover
stable manifests within a pass, changed trees between passes, restart recovery,
legacy cursors and history allocation. A new regression removes the allowed
root after the first page and proves a cached continuation is refused with no
result. Root permission is checked before the cached manifest or known hashes.
Moved comments no longer carry personal machine measurements. The scan wire
format, page bounds and extraction behavior remain unchanged. Retrieval/model
separation, source reset generations and performance measurements remain open.

Scanner validation: computer and all agent packages pass with the race detector
against the disposable database; both binary builds, repository lint and
...
Conversation-memory decomposition: remember.go now handles queueing and job
orchestration. Retrieval returns rememberMaterial through the existing database
interface, buildRememberPrompt constructs the unchanged request, and
parseRememberAnswer preserves the external JSON fields and empty-answer fallback.
Preparation helpers resolve the requested fact shape and perform embedding calls;
openRememberedPages and applyRememberedFacts name the existing persistence phases.
Evidence checks, replacement eligibility, folding and contact identity keep their
existing behavior in separate files.

A parsed-function comparison finds 33 original function bodies unchanged. The
two changed entry points delegate their original steps to the new helpers.
A temporary compatibility test compared the old prompt-construction code with
the new builder for empty/populated retrieval context, long Unicode input and
text resembling a closing delimiter; all rendered prompts were identical.
The temporary duplicate was removed after verification. Permanent response tests
cover the wire fields, prose, malformed JSON and discarding a partially decoded
answer. The full agent race suite passes, including fake-model retention,
deduplication, evidence, link and replacement rollback fixtures. Both builds,
repository lint and gogolint pass. Moved comments no longer carry personal graph
...
Recall review traced the open transaction to graphView, which previously wrapped
all GraphQL execution. A helper-only change could not release that connection.
HTTP queries now execute with root-resolver transactions installed at schema
construction. Each ordinary root field reads current account state and effective
permissions in its own transaction. Mutations keep their request transaction;
subscriptions retain their existing path. Recall is the query with separate
short authorization phases, independent graph reads and model work outside SQL.
It rechecks current account state, permission and active agent before returning.

HTTP integration tests use the real schema and a local embedding provider. They
exercise recall under an alias and directive, denied permission, skipped recall,
permission revoked by the provider, a disabled account and a disabled agent.
Provider calls must see no active request transaction, no nested transaction may
be acquired, and revoked results must not be returned. A separate transport test
checks two query aliases receive two transactions, two mutation fields retain
one, and a skipped query field acquires none. Existing recall ranking and usage
bookkeeping contracts remain covered by the agent suite. Remaining dream
extraction, measurements, lifecycle review and deployment gates are still open.

The first full-suite run found the new authorization helper was absent from the
source-level authorization inventory. It is now named requireRecallPerson and
registered there as the short-transaction form of requireAgentPerson. The HTTP
regression also checks an anonymous caller, which must receive no recalled
content and trigger no model call. The focused authorization and HTTP tests pass;
complete database-image runs are repeated after this correction.

The final query-isolation regression caught a partial-write bug in the first
wrapper: it retained a resolver error for GraphQL but returned nil to the
transaction callback, committing a successful write before the error. The
wrapper now returns the resolver error from the callback. The regression creates
a user, rejects a duplicate, checks that neither write persists, and verifies
that a sibling root field still succeeds. The focused race run passes after
this correction; full database-image validation is repeated for the final code.

Final HTTP query validation passes: the standard PostgreSQL suite runs 2,028
tests with two expected skips and 42.3% aggregate coverage; the vector suite
runs 2,028 tests with one expected skip and 42.4% coverage. Both binaries build,
repository lint and gogolint pass, and the focused HTTP/authorization race tests
pass. Added and untracked text passes the privacy scan. The lint invocation
required adding the installed Node runtime to PATH; its rerun passed. No dashboard
rendering changed in this step. Full deployment and final Chrome gates remain
open alongside the remaining milestones.

Dream-processing extraction keeps runDream and its shared pacing bounds in
internal/agent/dream.go. Scheduling, model calls, budget accounting, document
digestion, timeline writing, consolidation, organization and page splitting each
have a separate file beside it. The receiver-qualified AST comparison reports
all 37 original function bodies unchanged. Prompts, response contracts, phase
order, transaction scopes and retry behavior are preserved. Historical personal
comments are removed, and the digest failure comment now accurately distinguishes
committed page preparation from fact filing.

Review found remaining bookkeeping concerns: digest completion marks documents
read separately from dream progress, and splitPage increments its local moved
count inside a transaction before commit. These require focused failure tests
and fixes after this behavior-preserving extraction. Digest retrieval, request
parsing and application also still need narrower boundaries within their file.
This change does not close the remaining model-adapter or benchmark gates.

Dream extraction validation: all agent packages pass with the race detector
against the disposable vector database, including the existing whole-dream,
budget, digest splitting, tools, author evidence, merge and attachment regressions.
...
Dream bookkeeping now uses a shared ten-second completion deadline for read
markers, malformed-answer counts, progress, attachment declines and picture
descriptions. Each operation retains context values and may finish after the
run is canceled, but waiting for the database is bounded. The limit matches
job completion bookkeeping. This bounds each operation, not the total time for
all concurrent batches to finish; the document/progress atomicity finding is
still open.

Regression coverage simulates a stalled connection acquisition for each of the
five operations and checks that cancellation remains detached, audit context is
preserved, and a deadline exists. Every operation returns when that deadline
expires. A real-database regression cancels its parent first and verifies that
read markers, malformed-answer attempts, attachment declines, picture chunks
and dream progress still persist. A failed malformed-answer write must leave
the document eligible for another attempt.

Bookkeeping validation: the full standard PostgreSQL suite passes 2,042 tests
with two expected skips and 42.4% aggregate coverage. All agent packages pass
with the race detector against the vector database; the focused five-operation
deadline and canceled-parent persistence regressions pass. Both binaries build,
repository lint and gogolint pass, and privacy checks cover added and untracked
text. Temporarily replacing the completion timeout with an unbounded detached
...
Embedding selection now belongs to the model registry. Its typed result carries
the client, provider model name, stored vector identity and requested dimensions
from the same startup configuration. Graph/chunk embedding, mailbox embedding,
mailbox meaning search and mailbox backfill use that selection. Disabled or
absent registries remain inactive, while pending model changes take effect when
the registry is reopened. Live feature permissions and request timeouts remain
live. Existing vectors that were already mislabeled are not identifiable from
their stored values and are not automatically deleted by this change.

The regression changes the live model name and width after opening the registry,
and separately clears the live model name. It checks actual provider request
fields and stored mailbox, chunk and graph identities, mailbox search results,
and no backfill jobs for completed dimensioned vectors. A registry test checks
the old selection, the new selection after reopening, and disabled behavior.
Restoring the original implementation makes both integration cases fail; the
fixed files are restored and byte-compared before broader checks. The synthetic
sent-message cutoff is explicit so the future-dated fixture is included. Other
startup-versus-live model capability and prompt reporting remains part of the
remaining lifecycle/model audit.

Embedding-selection validation passes: the full standard PostgreSQL suite runs
2,073 tests with two expected skips and 42.5% aggregate coverage. Agent and LLM
packages pass with the race detector against the disposable vector database.
Both binaries build, repository lint and gogolint pass, and added-text privacy
review and staged secret checks are clean. The final focused regression is
repeated after the review correction that propagates invalid backfill selection
errors. No dashboard changes are included.

Periodic graph embedding now prepares a typed batch containing each vector and
the modification times of its input page and, where applicable, fact. The
database locks facts first and then pages, ordered by identifier within each
table, matching fact edits that record a revision on their page. It verifies
agent ownership, row existence, versions and fact-to-page identity before
writing. This protects the check/write interval without holding a transaction
during the provider call. A skipped stale result is not counted as written.
The whole accepted batch still shares a transaction and reports zero on failure.

The regression changes a page name, edits a fact, or deletes the page while the
local embedding provider is handling the request. It verifies no stale result,
continued progress for unaffected pages, exact committed counts and a retry that
embeds only changed rows. The original implementation restores stale vectors
after edits and loses all valid work after deletion; each case fails before the
fix. The fixed file is restored and byte-compared before broader validation.
This change covers periodic graph embedding. Existing-vector invalidation for
all page metadata changes and other prepared model writes remain in the model
side-effect audit; no claim of full graph audit completion is made.

Follow-up review identifies AskRun.NoteNode and AskRun.NoteFact in
internal/agent/graph_note.go as additional in-flight embedding writers that
still need the same guard. They remain explicit next work, alongside page
metadata invalidation; the current batch correction does not imply those
interactive paths have been fixed.

Guarded graph-batch validation passes: the full standard PostgreSQL suite runs
2,077 tests with two expected skips and 42.5% aggregate coverage. Agent and
database packages pass race tests against the disposable vector database,

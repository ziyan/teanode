...
- Decision: a source type is a YAML file like a skill, distributed through the same signed registry, in a `sources/` directory beside `skills/`, listed in the same index with a `kind` field.
  Rationale: `docs/decisions/20260911-skills-are-signed-and-run-where-they-can-do-least.md` chose one registry and one key on purpose: one trust path to audit. A type runs commands on a person's computer with nobody watching, which needs at least the trust a skill needs. A second registry would be a second key to guard for no gain.

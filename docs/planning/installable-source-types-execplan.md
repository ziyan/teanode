...
- Decision: a source type is a YAML file like a skill, distributed through a registry of its own, `github.com/teanode/teanode-sources`, laid out as `github.com/teanode/teanode-skills` is: an `index.json` listing each type with its version, URL, SHA-256 and signature, and `sources/<name>/source.md` for each type. It is signed with the same Ed25519 key as the skills registry.
  Rationale: types and skills are installed, reviewed and released on their own schedules, and a person browsing either should see only one kind of thing, so each gets its own repository and index. The key stays one: `docs/decisions/20260911-skills-are-signed-and-run-where-they-can-do-least.md` chose one key so that there is one secret to guard and one check in the binary, and a type runs commands on a person's computer with nobody watching, which asks at least as much trust as a skill. A second key would double what has to be guarded for no gain. That decision record gains a note saying the key now signs two indexes.
  Date/Author: 2026-09-23, the person (a separate repository), agent (the same key).
...
Revision note (2026-09-23): source types have a registry of their own, `github.com/teanode/teanode-sources`, instead of a `sources/` directory in the skills registry, at the person's request; it is signed with the same key, for the reason recorded in the Decision Log.

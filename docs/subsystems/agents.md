...
where the person can see it, change it and turn it off. One rule covers both:

    teanode mailbox rule add "Unwanted" \
      --when category:matches:^(phishing|junk)$ --move Junk --mark-read --stop

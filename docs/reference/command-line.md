...
| `teanode agent memory answers <file>` | answer each question of a set that has an `expectedAnswer`, from memory, from the sources or from both (`--from memory,sources,both`), and grade each answer against the expected one: a line a question and source — `memory changed-01 stale gave the old address` — then a score, the verdicts, the cost and the time an answer took, per source. Two model calls a question and source, as runs of kind `evaluate`; nothing is written to a conversation or the graph. `--json` for every answer. See `docs/evaluation/` |

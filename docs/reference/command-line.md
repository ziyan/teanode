...
| `teanode calendar request <request-id>` | check whether a calendar save completed, even after its event or calendar was deleted. `add` and `edit` print a request ID before saving; after an uncertain response, look it up here or retry identical fields with `--request-id`. An absent receipt may mean the original request is still in progress. Changed fields require a new ID |

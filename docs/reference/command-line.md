...
| `teanode agent knowledge list\|add\|pause\|resume\|sync\|allow\|remove` | the places your agent reads: `add "work" ~/work --computer laptop --under work`, `sync` reads one again now, `pause` keeps what it found. `--format` says how to read what is there: `files` (a tree of files, git-aware), `mattermost` (an export), `journal` (dated notes), or `records` (a folder of JSON lines any script can write, with a `refresh` script beside them the daemon runs before each scan — ask your agent to write one) |

...
| `teanode agent skill list\|search\|install\|update\|remove\|enable\|disable\|secret` | tools installed from the skill registry, for everyone on this server: `search` says what there is, `install weather` checks the signature and the hash before keeping it, `update` with no name installs every newer version there is. Installing needs `server:manage`. A skill that runs commands runs them on a computer you attached, asks first, and is never used by a run with nobody watching. `secret list\|set\|clear` is for the values a skill asks *you* for rather than the server: `secret set news NEWSAPI_KEY -` reads it from the terminal without echoing |

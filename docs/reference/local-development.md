...
## Build and test
    ...
    make lint           # golangci-lint plus the local naming checks
...
The sender has to be at a real domain that publishes mail servers and does
not ask for failures to be refused: every reserved example domain now
publishes a DMARC policy of `reject`, so a message "from" `someone@example.net`
is turned away at the door, which is correct and unhelpful, and
`webmail.example` above stands for a domain of your choosing. One with a
policy of `none` — most large webmail providers — is scored and accepted.

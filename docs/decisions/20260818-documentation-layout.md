...
Separately: `gogolint`, which enforces the naming and error-prefix conventions,
...
- `gogolint` not being in CI means a contributor without it installed is never
  blocked, at the cost of naming drift being caught at review rather than
  automatically. `.gogolint.yaml` registers this project's mail vocabulary so the

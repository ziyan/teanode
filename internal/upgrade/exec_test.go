...
// The in-place road gets the same second chance the staging road has.
//
// An automatic upgrade to a release that crashes before it serves used to
// leave a supervisor restarting the broken binary for ever: no marker, no
// backoff, nothing had failed, and mail down until somebody renamed a file by
// hand. The comment saying why there was no guard claimed there was no older
// binary to go back to — while swap had been keeping one beside it all along.
//
// Not parallel: it becomes the upgraded binary. See pretendToBe.
func TestAnInPlaceUpgradeThatDidNotStartGoesBack(t *testing.T) {
	directory := t.TempDir()
	executable := filepath.Join(directory, "teanode")
	if err := os.WriteFile(executable, []byte("the new binary"), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(executable+previousSuffix, []byte("the old binary"), 0o700); err != nil {
		t.Fatal(err)
	}
	pretendToBe(t, executable)

	// What runServer does before exec'ing an in-place upgrade.
	MarkTried("", executable)
	if _, err := os.Stat(InPlaceMarker(executable)); err != nil {
		t.Fatalf("the in-place upgrade was not marked: %s", err)
	}

	// And what the next start finds. It cannot be run here — it would replace
	// this test process — so the marker is the observable: taken off before
	// the fallback, so a binary that crashes cannot loop on this either.
	ExecPreviousIfLastStartFailed("0.2.0")
	if _, err := os.Stat(InPlaceMarker(executable)); !os.IsNotExist(err) {
		t.Errorf("the marker survived, so this would happen at every start: %v", err)
	}

	// Serving takes it off too, which is the ordinary path.
	MarkTried("", executable)
	Started("")
	if _, err := os.Stat(InPlaceMarker(executable)); !os.IsNotExist(err) {
		t.Errorf("a binary that served is still marked as untried: %v", err)
	}
}

// An exec that never happened is not a start that did not serve, on either
// road. Left marked, the in-place one would go back to the older binary at the
// next start for no reason at all.
func TestAFailedInPlaceExecTakesTheMarkOff(t *testing.T) {
	t.Parallel()
	...
	Untried("", executable)
	if _, err := os.Stat(InPlaceMarker(executable)); !os.IsNotExist(err) {
		t.Errorf("the mark stayed on a binary that was never run: %v", err)
	}
}

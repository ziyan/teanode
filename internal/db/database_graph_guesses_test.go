...
func TestMigration0088MarksOnlyTheNightsGuesses(t *testing.T) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		for _, page := range []struct {
			...
			{"things/gripper", models.NodeThing},
			{"places/osaka", models.NodePlace},
	...
	// dream's own actor.
	link(models.ActorDream, "people/alice-chen", "things/gripper", models.EdgeWorksOn,
		"she may have written the payload angle check",
		[]models.Evidence{{Kind: models.EvidenceDocument, Quote: "people/alice-chen → projects/portal → things/gripper"}})
	...
	link(models.ActorDream, "people/alice-chen", "places/osaka", models.EdgeLocatedIn, "perhaps", nil)
	link(models.ActorPerson, "people/alice-chen", "places/osaka", models.EdgeLocatedIn, "she moved there in 2019", nil)
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		dream.Proposals = []models.DreamProposal{
			{Kind: "linked", Path: "people/alice-chen", To: "places/osaka", Reason: "perhaps"},
	...
	for _, pair := range [][2]string{
		{"people/alice-chen", "things/gripper"},
		{"people/alice-chen", "projects/portal"},
		{"people/alice-chen", "places/osaka"},
	...
	if status := statusOf("after the backfill", "people/alice-chen", "things/gripper"); status != models.EdgeProposed {
	...
	if status := statusOf("after the backfill", "people/alice-chen", "places/osaka"); status != models.EdgeStated {
	...
	if status := statusOf("after reverting", "people/alice-chen", "things/gripper"); status != models.EdgeStated {
		t.Fatalf("reverting puts it back, not %q", status)
	}
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		edges, err := tx.ListAgentEdges(agentId, pages["things/gripper"].ID)

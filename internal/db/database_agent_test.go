...
func TestAgentQueueAndUsage(t *testing.T) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		if _, err := tx.FinishAgentJob(first.ID, claimed[0].ClaimID, &db.AgentJobOutcome{JobStatus: models.AgentJobQueued, ErrorMessage: "provider was busy", NotBefore: &retry, FailureCount: 1}); err != nil {
		...
		if _, err := tx.FinishAgentJob(first.ID, due[0].ClaimID, &db.AgentJobOutcome{JobStatus: models.AgentJobDead, ErrorMessage: "gave up", FailureCount: 2, FinishedAt: time.Now()}); err != nil {
			t.Fatalf("FinishAgentJob: %s", err)
		}

...
func TestGraphMakesTheFoldersAbeveAPage(t *testing.T) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		node, err := tx.PutAgentNode(&models.AgentNode{
			AgentID: agent.ID, Path: "work/northwind/dev/portal",
		...
		for _, path := range []string{"work", "work/northwind", "work/northwind/dev"} {
...
func TestGraphSearchFindsPagesAndFacts(t *testing.T) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		agent := graphAgent(t, tx)
		node, err := tx.PutAgentNode(&models.AgentNode{
			AgentID: agent.ID, Path: "things/marigold", Kind: models.NodeThing,
			Name: "Marigold", Summary: "The neighbour's sailing boat.",
		})
		if err != nil {
			t.Fatalf("PutAgentNode: %s", err)
		}
		...
		nodes, _, err := tx.SearchAgentGraph(agent.ID, "marigold", 10)
		if err != nil {
...
func TestGraphAProposedLinkIsConfirmedByStatingIt(t *testing.T) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		latch, err := tx.PutAgentNode(&models.AgentNode{AgentID: agent.ID, Path: "things/latch", Kind: models.NodeThing, Name: "Latch"})
		if err != nil {
			t.Fatalf("latch: %s", err)
		}
		if err := tx.PutAgentEdge(&models.AgentEdge{
			AgentID: agent.ID, FromID: alice.ID, ToID: latch.ID, Relation: models.EdgeWorksOn,
			Weight: 0.5, Status: models.EdgeProposed, Note: "she wrote the payload angle check",
		}); err != nil {
			t.Fatalf("PutAgentEdge: %s", err)
		}
		onlyEdge := func(where string) *models.AgentEdge {
			edges, err := tx.ListAgentEdges(agent.ID, latch.ID)
			if err != nil {
...
func TestGraphAFoldAndAStrikingKeepTheRow(t *testing.T) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		kept := add("Moved to Rivermouth in 2019.")
		repeated := add("In 2019 she moved to Rivermouth.")
...
//
// A path is one word to this index, deliberately, so that "notes/marigold"
...
func TestGraphMovingAPageReindexesWhatIsUnderIt(t *testing.T) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		if _, err := tx.PutAgentNode(&models.AgentNode{
			AgentID: agentId, Path: "burrow/marigold", Kind: models.NodeTopic, Name: "Marigold",
		}); err != nil {
			t.Fatalf("PutAgentNode: %s", err)
		}
		nestling, err := tx.PutAgentNode(&models.AgentNode{
			AgentID: agentId, Path: "burrow/marigold/nestling", Kind: models.NodeTopic, Name: "Nestling",
		})
		if err != nil {
			t.Fatalf("PutAgentNode: %s", err)
		}
		...
		if !graphSearchFinds(t, tx, agentId, "burrow/marigold/nestling", nestlingId) {
			t.Fatalf("a page is found by its own path")
		}
	})

	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		if _, err := tx.MoveAgentNode(agentId, "burrow/marigold", "warren"); err != nil {
			t.Fatalf("MoveAgentNode: %s", err)
		}
	})

	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		moved, err := tx.GetAgentNode(agentId, "warren/marigold/nestling")
		if err != nil || moved == nil || moved.ID != nestlingId {
			t.Fatalf("the page under the moved one has the new path: %v %s", moved, err)
		}
		if !graphSearchFinds(t, tx, agentId, "warren/marigold/nestling", nestlingId) {

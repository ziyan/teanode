...
// backgroundLast puts what a person is waiting for ahead of the reading.
//
// The queue was the order things were made in and nothing else. A server
// works three jobs at once; the nightly reading holds one of those for as
// long as it runs, and a pass over a large source holds another for up to
// forty minutes. So while a hundred and fifty thousand documents are being
// read -- which is days, not minutes -- a message arriving found four
// ingest jobs in front of it and waited an hour to be triaged. The agent
// looked dead to the person whose agent it is.
//
// Within each band the order is still the order they were made in, and
// when nobody is waiting -- which is almost always -- this is exactly what
// it was. The reading is not starved by it either: what goes first is
// short by nature, a triage or an embedding, not another long pass.
var backgroundLast = fmt.Sprintf(`CASE WHEN "kind" IN ('%s', '%s', '%s') THEN 1 ELSE 0 END`,

...
func TestSimpleCanonicalizer_CanonicalBody(t *testing.T) {
	...
	c, ok := mailparse.GetCanonicalizer(mailparse.CanonicalizationSimple)
	if !ok {
		t.Fatalf("failed to find canonicalizer")
	}
...
func TestRelaxedCanonicalizer_CanonicalizeHeader(t *testing.T) {
	...
	c, ok := mailparse.GetCanonicalizer(mailparse.CanonicalizationRelaxed)
	if !ok {
		t.Fatalf("failed to find canonicalizer")
	}

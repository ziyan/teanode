...
func TestAssociatePromptAsksForTheHonestNo(t *testing.T) {
	...
	prompt, err := render("associate.txt", map[string]any{
		...
		"To":         "things/latch — Latch: the payload latch",
		"Through":    []string{"projects/portal — Portal", "things/latch — Latch"},
		"Relations":  strings.Join(relations, ", "),
	})
	if err != nil {
		t.Fatalf("render: %s", err)
	}
	for _, wanted := range []string{"Alice Chen", "Latch", "works_on", `{"related": false}`} {

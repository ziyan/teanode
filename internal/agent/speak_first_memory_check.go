	...
	return speakFirstReason{
		...
		checkIn: func(ctx context.Context, tx db.Transaction, agent *models.Agent, owner *models.User, now time.Time, prepared map[string]string) (string, error) {
			return speakFirstMessage(owner, now, "It is time for a memory check: a few questions about things you remember about them, to learn how well your memory answers.") + askedLine(prepared) + strings.Join([]string{
				...
				"",
				"Put each with ask_user, the question being what you remember, and these choices, in their language: \"Yes, that's right\", \"It has changed\", \"Not sure\", \"Skip this one\", \"Stop for now\". They can also type their own answer, or choose to chat about it instead, which ends the card: then stop asking, and answer what they write next. When they say it has changed, ask what it is now, with ask_user and no choices. If ask_user says the question is left open, they have stepped away: end your turn, and their answer will come back as a new turn, where you record it and go on.",
				"",

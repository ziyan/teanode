...
func (self *Agent) memoryCheckReason() speakFirstReason {
	return speakFirstReason{
		...
		checkIn: func(ctx context.Context, tx db.Transaction, agent *models.Agent, owner *models.User, now time.Time, prepared map[string]string) (string, error) {
			return speakFirstMessage(owner, now, models.MemoryCheckOpening+": a few questions about things you remember about them, to learn how well your memory answers.", prepared) + strings.Join([]string{
				"Call memory_check with draft (load it with tool_search first if it is not among your tools). Say in one short line that you would like to check a few things you remember about them, and in this same turn, right after that line, put the first one with ask_user; do not end the turn after the line and wait for them to agree, since the card has \"Stop for now\". Then put the rest one at a time the same way.",
				"",

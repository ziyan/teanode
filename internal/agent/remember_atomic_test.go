...
func fileWithAFailingWrite(t *testing.T, failing string) (whatThePageKept, *models.AgentConversation) {
	...
	said := "The boat next door is called Puffin now, not Kittiwake."
		...
		if found := theirSentence.FindStringSubmatch(prompt); len(found) > 2 {
			answer = fmt.Sprintf(
				`{"facts": [{"path": "things/kittiwake", "node_kind": "thing", "node_name": "Kittiwake", "kind": "fact", "text": %q, "message_id": %q, "quote": %q}],`+
					` "supersedes": [{"path": "things/kittiwake", "number": 1}]}`,
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		...
		if page, err = tx.PutAgentNode(&models.AgentNode{
			AgentID: found.ID, Path: "things/kittiwake", Kind: models.NodeThing, Name: "Kittiwake",
		}); err != nil {
		...
		if _, err := tx.AddAgentFact(&models.AgentFact{
			...
			Text: "The boat next door is called Kittiwake.",
		}); err != nil {

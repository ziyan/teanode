...
// "Ran Liao ran liao example net" is nobody.
...
func machineLooking(author string) bool {
	...
	// A word with a number stuck on the end of it: controller1093.
...
func TestTheAuthorOfACommitGetsAPageAndALink(t *testing.T) {
	...
	documents := fileAuthoredCommits(t, database, run, []authoredCommit{
		{subject: "Obtain certificates without a cloud account", author: "Ran Liao <ran.liao@example.net>"},
	})
	...
	if made := prompts(); len(made) == 0 || !strings.Contains(made[0], "— by Ran Liao <ran.liao@example.net>") {
		t.Fatalf("the reading is shown who wrote the commit:\n%s", strings.Join(made, "\n"))
	}

	pages := peopleOf(t, database, run)
	page := pages["people/ran-liao"]
	if page == nil {
...
func TestAnAuthorThatReadsAsAMachineGetsNoPage(t *testing.T) {
	...
	documents := fileAuthoredCommits(t, database, run, []authoredCommit{
		...
		{subject: "Re-run the fixtures", author: "controller1093@example.net"},
		{subject: "Sign the release archives", author: "Ran Liao <ran.liao@example.net>"},
	})
	...
	pages := peopleOf(t, database, run)
	if len(pages) != 1 || pages["people/ran-liao"] == nil {
...
func TestTheReadingIsToldTheAuthorOfAnItemGetsAPage(t *testing.T) {
	...
	rendered, err := render("digest.txt", map[string]any{
		"PersonName": "Alice", "Items": "[d1] Obtain certificates — by Ran Liao — 17 Sep 2026", "Most": 5,
	})
	if err != nil {
	...
	object, err := render("digest_object.txt", map[string]any{
		"PersonName": "Alice", "Reading": "Ran Liao wrote the certificate change.",
	})
	if err != nil {

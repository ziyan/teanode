...
func TestNotingSaysWhenThePageAlreadySaysIt(t *testing.T) {
	...
	first := call(`{"action":"note","path":"things/kittiwake","kind":"thing","name":"Kittiwake","text":"The neighbour's boat, repainted every spring."}`)
	if !strings.Contains(first, "things/kittiwake#1") {
	...
	page := call(`{"action":"get","path":"things/kittiwake"}`)
	if !strings.Contains(page, "Kittiwake") || !strings.Contains(page, "#1") {
		t.Fatalf("the page reads back: %q", page)
	}

	// Now there is a twin.
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		node, err := tx.GetAgentNode(run.agent.ID, "things/kittiwake")
		if err != nil || node == nil {
	...
	second := call(`{"action":"note","path":"things/kittiwake","text":"Kittiwake gets a coat of paint each spring."}`)
	if !strings.Contains(second, "things/kittiwake#1") || !strings.Contains(second, "already says") {
		t.Fatalf("it should name the one it repeats: %q", second)
	}
	if !strings.Contains(second, "forget things/kittiwake#2") {

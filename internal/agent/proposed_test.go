...
func TestAWalkedLinkIsProposedAndNotStated(t *testing.T) {
	through := []string{"people/alice-chen — Alice Chen", "projects/portal — Portal", "things/latch — Latch"}
...
func TestAProposedLinkReadsAsAGuess(t *testing.T) {
	edge := &models.AgentEdge{
		...
		ToPath: "things/latch", ToName: "Latch",
		Note:   "she wrote the payload angle check",
		Status: models.EdgeProposed,
	}
	want := "perhaps related to Latch (things/latch) — she wrote the payload angle check (the agent's guess)"
	...
	if got := edge.Sentence("things/latch", false); !strings.HasPrefix(got, "perhaps related to Alice Chen") {
		t.Fatalf("from the latch's page: %q", got)
	}

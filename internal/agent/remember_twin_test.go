...
// "Marigold is the neighbour's boat" nine times over, so that the page
...
func TestRememberingTheSameThingTwiceKeepsItOnce(t *testing.T) {
	...
	facts, everyRow := rememberOnTwoDays(t, []string{
		"Marigold is the neighbour's boat, and they repaint it every spring.",
		"Marigold is the neighbour's boat, and they repaint it every spring.",
	})
	if len(facts) != 1 {
...
func TestRememberingTwoDifferentThingsKeepsBoth(t *testing.T) {
	facts, everyRow := rememberOnTwoDays(t, []string{
		"Marigold is the neighbour's boat, and they repaint it every spring.",
		"Marigold is moored at the pier at the end of the towpath.",
	})
	if len(facts) != 2 {
...
func TestRememberingAChangedAmountKeepsBoth(t *testing.T) {
	facts, _ := rememberOnTwoDays(t, []string{
		"Marigold costs the neighbours 4200 a year to keep afloat.",
		"Marigold costs the neighbours 3100 a year to keep afloat.",
	})
	if len(facts) != 2 {
...
func rememberOnTwoDays(t *testing.T, said []string) ([]*models.AgentFact, []*models.AgentFact) {
	...
	dbtest.RunTransactionOn(t, database, func(tx db.Transaction) {
		node, err := tx.GetAgentNode(found.ID, "things/marigold")
		if err != nil || node == nil {
...
//
// Enough to tell "the neighbour's boat Marigold" from "the invoice is
...
func writeMeaning(writer http.ResponseWriter, request *http.Request) {
	...
	for index, text := range body.Input {
		...
		// on top of a page about a latch often enough to matter.
		const width = 512

...
func TestRehearsalCannotCallAFailedRetrievalAGap(t *testing.T) {
	...
	if got := worker.canAnswerFromMemory(context.Background(), run, &dreamBudget{allowed: 1000}, "what did I promise the Rivermouth team?"); got != rehearsalUnknown {
...
func TestRehearsalCallsAnEmptyGraphAGap(t *testing.T) {
	...
	if got := worker.canAnswerFromMemory(context.Background(), run, &dreamBudget{allowed: 1000}, "what did I promise the Rivermouth team?"); got != rehearsalGap {

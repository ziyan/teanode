...
tools:
  - name: get_weather
    description: Get weather forecast for a US location (free-form input like "Fairhaven, RI" or "Chicago, IL"). Returns geocoded coordinates, current conditions, and today's forecast from NWS.
    type: workflow
    timeout: 30
    parameters:
      type: object
      properties:
        location:
          type: string
          description: "Free-form location (e.g. 'Fairhaven, RI', 'Chicago, IL', 'Portland, OR')"
      required: ["location"]
    steps:
      - name: geocode
        type: http

...
//
// And `serve`, which goes the other way: this command line answering the
// protocol so that a harness here can use the agent's own tools.

func newAgentMCPCommand() *cli.Command {
	return &cli.Command{
		Name:  "mcp",
		Usage: "the connected servers your agent reaches out to, and serving your agent's own tools to a program here",
		Commands: []*cli.Command{
			...
			// And the other direction: this server answering the protocol
			// for a program on this machine.
			newAgentMCPServeCommand(),
		},
	}
}

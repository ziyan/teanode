...
func runAgentGraphGet(ctx context.Context, command *cli.Command) error {
	...
	for _, edge := range page.Edges {
		// Every relation, part_of included. It was skipped here, with
		// nothing rendering it anywhere else, so a hundred and eight links
		// somebody had deliberately made -- "portal is part of northwind" --
		// could not be seen on either page they joined. The guess was
		// presumably that part_of says what the folder already says, and
		// it does not: where a page is filed is its parent, and this is a
		// page saying it belongs to something that may be filed anywhere.
		//
...
func runAgentGraphMove(ctx context.Context, command *cli.Command) error {
	if command.Args().Len() < 2 {
		return fmt.Errorf("give a page and where it goes: teanode agent memory move notes/marigold things")
	}
	connection, err := openClient(command)
	if err != nil {
		return err
...
func runDreamLog(ctx context.Context, command *cli.Command) error {
	...
	for _, dream := range dreams {
		...
		if dream.Backlog > 0 {
			...
			// standing in for one. The line said "at this pace" while
			// dividing by a constant two thousand, so a night that read
			// four thousand two hundred reported twice as many nights
			// left as it had -- and the person watching a hundred and
			// fifty thousand documents go by was reading that line to
			// decide whether to wait up.
			//
			// A run still going has not set its pace yet: what it has
			// read so far is what it has read so far, and dividing by
			// that early says ten times too many. That line says what
			// waits and leaves the counting to the run before it.
			if needed, known := dreamsLeft(dream.Backlog, dream.Digested, dream.FinishedAt != nil); known {
				_, _ = fmt.Fprintf(command.Writer, "  %d still waiting, about %d dream(s) at this pace\n", dream.Backlog, needed)
			} else {
				_, _ = fmt.Fprintf(command.Writer, "  %d still waiting\n", dream.Backlog)
			}
		}
		if dream.Coarse {
...
// dreamsLeft is how many more nights the backlog is, at the pace of the
// night just described, and whether that can be said at all.
//
// A run still going has no pace yet: what it has read so far is what it
// has read so far, and a night a minute old would report a hundred times
// the nights it needs. Its line says what waits and leaves the counting
// to the night before it.
func dreamsLeft(backlog, digested int, finished bool) (int, bool) {
	if !finished || digested <= 0 || backlog <= 0 {
		return 0, false
	}

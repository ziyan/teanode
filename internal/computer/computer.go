...
type Options struct {
	...
	//
	// The server can only report that this end did not answer within ten
	// minutes. Which request, and whether it ran at all or never arrived,
	// is known here and nowhere else -- and a source that timed out every
	// pass for an evening could not be told apart from one that was never
	// asked, because the program that was or was not asked said nothing.
...
func withDefaults(options *Options) *Options {
	...
	if filled.Notice == nil {
		filled.Notice = func(string) {}
	}
	return filled
}

// whatWasAsked names the thing a request is about, so that a line about
// a request that took ten minutes says which source or which file.
//
// What it will say is fixed here and small on purpose: this goes to the
// person's terminal, and a request carries whatever the server sent --
// the text of a command, the contents of a file. A path and a cursor are
// enough to find the pass again, and nothing else is printed.
func whatWasAsked(args json.RawMessage) string {
	var asked struct {
	...
	case asked.After == "":
		return " " + where
	default:
		return " " + where + " after " + asked.After
	}
}

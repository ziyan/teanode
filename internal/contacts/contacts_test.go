...
func TestAPostalAddressIsReadBack(t *testing.T) {
	fromAPhone := "BEGIN:VCARD\r\nVERSION:3.0\r\nUID:u\r\nN:Lovelace;Ada;;;\r\nFN:Ada Lovelace\r\n" +
		"item2.ADR;TYPE=HOME;TYPE=pref:;;12 Juniper Way;Fairhaven;GA;10001;United States\r\n" +
		"item2.X-ABADR:us\r\nTEL;TYPE=CELL:+1 (555) 0100-0199\r\nEND:VCARD\r\n"
	parsed, err := Parse([]byte(fromAPhone))
	if err != nil {
		t.Fatalf("parse: %s", err)
	}
	...
	if address.Street != "12 Juniper Way" || address.Locality != "Fairhaven" ||
		address.Region != "GA" || address.PostalCode != "10001" || address.Country != "United States" {
		t.Fatalf("its components: %+v", address)
	}
	if address.Label != "home" {
		t.Errorf("what the phone calls it: %q", address.Label)
	}
	if written := address.Written(); written != "12 Juniper Way, Fairhaven, GA, 10001, United States" {

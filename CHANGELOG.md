...
- Sources that read the same computer take turns page by page; a source starting a pass no longer waits for hours behind ones with long pages. (#167)
...
- The reader no longer refuses a file because of what it is called or what
  it contains. A personal archive is read whole; only size, an unreadable
  kind, or the source's own globs hold a file back. (#176)

## [0.44.4] - 2026-09-20

### Fixed

- A page filed under a page of the same name now resolves to that page,
  instead of splitting one subject across two. (#173)

## [0.44.3] - 2026-09-20

### Fixed

- A message arriving while the agent is reading a large source is triaged
  straight away instead of queueing behind hours of background work. (#174)

## [0.44.2] - 2026-09-20

### Fixed

- `teanode agent dream log` works out the nights left from what the night
  actually read, instead of from a fixed two thousand, which overstated them
  by about double. (#172)
...
- A source whose documents are slow to read, such as a Drive of spreadsheets
  and presentations where each one costs a conversion, makes progress
  instead of timing out at the same place on every pass. (#171)

## [0.44.0] - 2026-09-20

### Added

- `teanode computer daemon --verbose` says what the agent asked for and how
  long each request took. (#170)

### Fixed

- A document whose text carries a NUL byte is kept, with the byte taken out,
  instead of being dropped on every pass. (#170)

## [0.43.5] - 2026-09-20

### Fixed

- On a phone, the last row of a list is no longer stuck underneath the
  button that opens the agent, where it could be seen but not pressed. (#169)

## [0.43.4] - 2026-09-20

### Fixed

- The nightly reading no longer files a second page for a subject that
  already has one where the two names say the same thing, which was
  scattering one subject across a dozen pages. (#166)
- A batch of documents whose answer the night could not make sense of is
  tried once more on a later night instead of being marked read
  immediately, and is findable afterwards either way. (#166)

## [0.43.3] - 2026-09-20

### Fixed

- A picture the agent cited says which channel it came from, rather than
  the thread identifier the chat service gave it. (#167)
- Reading a page from the command line shows its `part_of` links, which
  were being left out entirely. (#167)

## [0.43.2] - 2026-09-20

### Fixed

- The recall evaluation now builds the same knowledge index a real turn
  does, so a question is graded against what the agent would actually have
  had in front of it rather than against less. (#168)

## [0.43.1] - 2026-09-20

### Fixed

- A fact is no longer retired just because a run named it: taking a line off a
  page needs a replacement that stands at least as firmly, or the person's own
  words retracting it. (#163)
- Two statements worded the same but about different dates are kept as two,
  where the later used to be dropped and the page went on showing the older
  date. (#163)
- A model answering the nightly file selection with an error object no longer
  declines that whole batch of files for good. (#163)
- The nightly rehearsal no longer reports that memory can answer a question
  using a fact that has been struck or superseded. (#163)

## [0.43.0] - 2026-09-20

### Added

- The reading progress says what the nights behind its pace cost and roughly
  what the rest will cost, where the models in use have prices configured. (#164)

### Fixed

- A dream the server restarts under now shows what it read and filed before
  it was cut off, rather than showing nothing. (#164)
- The bootstrap row no longer says the reading queue is empty while it is
  still counting it. (#164)

## [0.42.5] - 2026-09-20

### Changed

- The ways into the knowledge graph make way for the search box while it is
  in use, instead of moving to a second row and pushing the list down. (#165)

## [0.42.4] - 2026-09-20

### Changed

- The row naming the folder you are in stays in view while its pages
  scroll, and names the top of the tree as a way back to it. The redundant
  "Knowledge" label above the top level is gone. (#162)
- The lightbox's "Fit" is drawn as a button, like the zoom controls beside
  it. (#162)
...
- Asking for more of the audit log or of a mailbox's subscriptions is now
  a button, matching every other list. The audit log shows the count
  beside the button rather than inside it. (#158)

## [0.42.2] - 2026-09-20

### Fixed

- Spreadsheets and presentations read from a person's files or their
  archives are indexed with their text in them. They were converted with a
  filter that only handles word processor documents, which silently
  produced nothing, so they were searchable by name alone. (#161)

## [0.42.1] - 2026-09-20

### Changed

- The search box above the knowledge pages is now the same height as the
  buttons beside it, and opens to the full width of the column while it has
  focus or anything typed in it. (#160)

## [0.42.0] - 2026-09-20

### Added

- TeaNode now answers the Model Context Protocol at `/api/v1/mcp`, so a coding
  harness or an editor can use a person's agent tools and ask their agent
  questions, authenticated with an ordinary API token. `teanode agent mcp
  serve` connects one on the same machine without pasting a token. What a
  caller is offered is what that account's permissions already allow. (#159)

## [0.41.3] - 2026-09-19

### Changed

- A folder in the Knowledge page lists its pages by what changed most
  recently rather than alphabetically. (#156)

## [0.41.2] - 2026-09-19

### Changed

- The control that loads more of a list is drawn as a button rather than
  an underlined link. (#155)

## [0.41.1] - 2026-09-19

### Fixed

- An office document is converted instead of silently producing nothing,
  and the converter no longer outlives its timeout holding temporary files. (#157)
...
- The Knowledge page's search box and the three ways into what your agent
  knows share one row, the way the mail list does. (#146)
...
- Equipment on your own network can be reached by a skill. The address guard
  ...
  UniFi Protect skill answered that it could not reach `192.168.255.254`.
...
- Your agent finds what it remembers by meaning, not only by the words
  ...
  Marigold. Each memory gets a vector when it is written, on the

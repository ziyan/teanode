...
const PAGE = `query ($path: String!) {
  AgentGraphPage(path: $path) {
    ...
    attachments { documentId name contentType channel path }
...
export function KnowledgePage() {
  ...
  // Where you are, and the ways back out of it.
  //
  // Above the rows rather than among them, because it was the first thing
  // to scroll away and it is the only way up: walking into a long folder
  // and then wanting to leave it meant scrolling back to the top first.
  //
  // Nothing at all at the top level. It said KNOWLEDGE there, under a page
  // already titled Knowledge, which is a word spent on saying where you
  // are when you are nowhere in particular.
  //
...
function AttachmentEvidence({ attachment }: { attachment: Attachment }) {
  ...
  // channel beside it is the part that answers that: "loki-hw-dev",
  // "paltac_tochigi_onsite".
  const where = attachment.channel.trim()

...
// The other side of the connected servers: this server answering the same
// protocol, so a program the person runs can use their agent's tools. It is
// three things to copy and nothing to configure, because the endpoint is
// always there and an API token is the whole of the setup.
function HarnessCard() {
  const { t } = useTranslation()
  // Where this dashboard is served from is where the endpoint is. Read from
  // the page rather than configured, so it is right on a development server,
  // behind a proxy, and on whatever port the operator chose.
  const endpoint = `${window.location.origin}/api/v1/mcp`
  const remote = `claude mcp add --transport http teanode ${endpoint} --header "Authorization: Bearer YOUR_TOKEN"`
  const local = 'claude mcp add teanode -- teanode agent mcp serve'
  const lines: { key: string; title: string; value: string }[] = [
    { key: 'endpoint', title: t('agent.harnessEndpoint'), value: endpoint },
    { key: 'remote', title: t('agent.harnessRemote'), value: remote },
    { key: 'local', title: t('agent.harnessLocal'), value: local },
  ]
  return (
    <SettingsSection card title={t('agent.harness')} description={t('agent.harnessHint')}>
      {lines.map((line) => (
        <SettingsRow
          key={line.key}
          title={line.title}
          subtitle={
            <div className="value-row">
              <Tooltip label={line.value}>
                <span className="value-clamp">{line.value}</span>
              </Tooltip>
            </div>
          }
          actions={<CopyIconButton value={line.value} label={t('common.copy')} />}
        />
      ))}
    </SettingsSection>
  )
      ...
      <SettingsRow
        ...
        subtitle={
          reading ? (
            ...
            // Not yet is not nothing.
            //
            // The row said "Nothing waits to be read" whenever it had no
            // figure to show, and it has no figure to show while it is
            // still asking -- so a page opened on a machine with a hundred
            // thousand documents queued said, for as long as the query
            // took, that there was nothing to do. Beside a tag saying it
            // was bootstrapping, which is what a person does about a
            // backlog.
            t('common.loading')
          ) : undefined
          // And where the query failed there is still no figure, so there
          // is still nothing true to say about the backlog. The row keeps
          // its name, its tag and its button and says none of it.
        }
  ...
  return (
    <span className="agent-reading">
      ...
      <span>
        ...
        {left ? ` · ${left}` : ''}
        {/* What the rest comes to, beside how long it takes. Hours are
            not what somebody watching a hundred and fifty thousand
            documents go by is deciding about; the bill is. Shown only
            where there is a price to work it out from -- a model somebody
            runs themselves has none, and nothing at all is a better
            answer there than a confident zero. */}
        {reading.costLeft > 0
